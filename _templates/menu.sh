...
# https://github.com/jpedro/menu.sh
    ...
    case "$key1" in
      $ESCAPE_SEQ)
        ...
        then
          ...
          case "$key3" in
            $ARROW_UP)
              [[ $MENU_INDEX -eq 0 ]] && menu_choose $MENU_COUNT || menu_choose $((MENU_INDEX - 1));;
            $ARROW_DOWN)
              [[ $MENU_INDEX -eq $(( $MENU_COUNT - 1 )) ]] && menu_choose 0 || menu_choose $((MENU_INDEX + 1));;
          esac
        fi
        ;;
